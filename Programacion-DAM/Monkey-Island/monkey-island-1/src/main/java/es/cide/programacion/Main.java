...
public class Main {
    ...
    String[] insultos = { // Array de los insultos del pirata
        ...
        System.out.println("Pon el nombre de tu Heroe");
        String nomHeroi = sc.nextLine(); // Pone el nombre
       ...
       Heroi heroi = new Heroi(nomHeroi); // Se crea un objeto heroe

        for (int i = 0; i < illa.getNumPiratas() && vida; i++) { // Enterara en el for las mismas veces que piratas te enfrentes

            Pirata pirata = illa.vullUnPirata(i); // Coge un pirata de los que hay aleatoriamente
            System.out.println("Tu enfrentamiento sera contra " + pirata.getNomPirata() + " y tiene: " + pirata.getVidaPirata() + " de vida"); // Te dira el nombre del pirata y su vida

            while (pirata.getVidaPirata() > 0 && heroi.getVida() > 0) {// Entra en el while hasta que uno de los dos llegue a vida 0
                ...
                String respuestaHeroe = heroi.defensar();
                
                if (pirata.replica(respuestaHeroe)) {// Aqui entrara si el heroe gana la discusion
                    System.out.println("Correcto");
                    pirata.restarVidaPirata();
                    System.out.println("Al pirata le queda "+ pirata.getVidaPirata() +" de vida"); // Entrara al metodo de vida del pirata y le restara 1
                
                    if (pirata.vidaPirata <= 0) {// Aqui entrara si el pirata ha muerto
